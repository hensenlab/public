...
class Keithley2400(VisaInstrument):
    ...
    def ramp_down(self,**kw):
        """
        ramp to zero voltage or current, depending on current mode.
        """
        
        self.ramp_to(setpoint=0,**kw)
    
    def ramp_to(self, setpoint, ramp_time = 1, sleep_time=0.037):
        """
        ramp to sepoint voltage or current, depending on current mode.
        ...
        """
        if self.mode.get() == 'CURR':
            current_setpoint = self.curr.get()
            ramp = np.linspace(current_setpoint,setpoint,int(np.ceil(ramp_time/sleep_time)))
            for c in ramp:
                self.curr.set(c)
                time.sleep(sleep_time)
        elif self.mode.get() == 'VOLT':
            voltage_setpoint = self.volt.get()
            ramp = np.linspace(voltage_setpoint,setpoint,int(np.ceil(ramp_time/sleep_time)))
            for v in ramp:
                self.volt.set(v)
                time.sleep(sleep_time)
