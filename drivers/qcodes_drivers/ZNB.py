"""
Almost identical to https://microsoft.github.io/Qcodes/_modules/qcodes/instrument_drivers/rohde_schwarz/ZNB.html
...
class FixedFrequencyTraceIQ(MultiParameter):
    ...
    def __init__(
        self, name: str, instrument: "ZNBTrace", npts: int, bandwidth: int
    ) -> None:
        super().__init__(
            name,
    ...
    def get_raw(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        """
        assert isinstance(self.instrument, ZNBTrace)
...
class FixedFrequencyPointIQ(MultiParameter):
    ...
    def __init__(self, name: str, instrument: "ZNBTrace", **kwargs) -> None:
        super().__init__(
            name,
            instrument=instrument,
            names=("I", "Q"),
...
class ZNBTrace(InstrumentChannel):
    def __init__(
        ...
        """
        super(ZNBTrace, self).__init__(parent, name)

        self._ch = channel
        # Additional wait when adjusting instrument timeout to sweep time.
        self._additional_wait = 100

        if existing_trace_to_bind_to is None:
            self._tracename = f"Trc{name}"
            if vna_parameter is None:
                vna_parameter = name
            self._vna_parameter = vna_parameter
            # self.write(
                # f"CALC{self._ch}:PAR:SDEF"
                # f" '{self._tracename}', '{self._vna_parameter}'"
            # )

                    # initialising channel
            #self.write(f"SENS{self._ch}:SWE:TYPE LIN")
            #self.write(f"SENS{self._ch}:SWE:TIME:AUTO ON")
            #self.write(f"TRIG{self._ch}:SEQ:SOUR IMM")
            #self.write(f"SENS{self._ch}:AVER:STAT ON")

        else:
            traces = self._parent.ask(f"CONFigure:CHANnel{self._ch}:TRACe:CATalog?")
        ...
        self.add_parameter(
            name="vna_parameter",
            label="VNA parameter",
            get_cmd=f"CALC{self._ch}:PAR:MEAS? "
                    f"'{self._tracename}'",
            get_parser=self._strip,
        )    

        self._vna_parameter = self.vna_parameter.get()
        ...
        self.add_parameter(
            ...
            label="Power",
            unit="dBm",
            get_cmd=f"SOUR{self._ch}:POW?",
            set_cmd=f"SOUR{self._ch}:POW {{:.4f}}",
            get_parser=float,
        ...
        self.add_parameter(
            ...
            label="Averages",
            unit="",
            get_cmd=f"SENS{self._ch}:AVER:COUN?",
            set_cmd=f"SENS{self._ch}:AVER:COUN {{:.4f}}",
            get_parser=int,
            vals=vals.Ints(1, 5000),
        )
        self.add_parameter(
            name="start",
            get_cmd=f"SENS{self._ch}:FREQ:START?",
        ...
        self.add_parameter(
            name="stop",
            get_cmd=f"SENS{self._ch}:FREQ:STOP?",
        ...
        self.add_parameter(
            name="center",
            get_cmd=f"SENS{self._ch}:FREQ:CENT?",
        ...
        self.add_parameter(
            name="span",
            get_cmd=f"SENS{self._ch}:FREQ:SPAN?",
        ...
        self.add_parameter(
            name="npts",
            get_cmd=f"SENS{self._ch}:SWE:POIN?",
            set_cmd=self._set_npts,
            get_parser=int,
        )
        self.add_parameter(
            name="status",
            get_cmd=f"CONF:CHAN{self._ch}:MEAS?",
            set_cmd=f"CONF:CHAN{self._ch}:MEAS {{}}",
            get_parser=int,
        )
        self.add_parameter(
            name="format",
        ...
        self.add_parameter(
            ...
            get_cmd=f"SENS{self._ch}:CORR:EDEL2:TIME?",
            set_cmd=f"SENS{self._ch}:CORR:EDEL2:TIME {{}}",
            get_parser=float,
            unit="s",
        )
        self.add_parameter(
            name="sweep_time",
            label="Sweep time",
            get_cmd=f"SENS{self._ch}:SWE:TIME?",
            get_parser=float,
            unit="s",
        )
        self.add_parameter(
            name="sweep_type",
            get_cmd=f"SENS{self._ch}:SWE:TYPE?",
        ...
        self.add_parameter(
            name="cw_frequency",
            get_cmd=f"SENS{self._ch}:FREQ:CW?",
        ...
        self.add_function(
            "set_electrical_delay_auto", call_cmd=f"SENS{self._ch}:CORR:EDEL:AUTO ONCE"
        )
        self.add_function(
            "autoscale",
    ...
    def _get_format(self, tracename: str) -> str:
        self.write(f"CALC{self._ch}:PAR:SEL '{tracename}'")
        return self.ask(f"CALC{self._ch}:FORM?")
    ...
    def _set_format(self, val: str) -> None:
        ...
        self.write(f"CALC{self._ch}:PAR:SEL '{self._tracename}'")
        self.write(f"CALC{self._ch}:FORM {val}")
    ...
    def _set_start(self, val: float) -> None:
        self.write(f"SENS{self._ch}:FREQ:START {val:.7f}")
        stop = self.stop()
        if val >= stop:
    ...
    def _set_stop(self, val: float) -> None:
        ...
        if val <= start:
            raise ValueError("Stop frequency must be larger than start "
                             "frequency.")
        self.write(f"SENS{self._ch}:FREQ:STOP {val:.7f}")
    ...
    def _set_npts(self, val: int) -> None:
        self.write(f"SENS{self._ch}:SWE:POIN {val:.7f}")
        self.update_lin_traces()
        self.update_cw_traces()

    def _set_bandwidth(self, val: int) -> None:
        self.write(f"SENS{self._ch}:BAND {val:.4f}")
        self.update_cw_traces()

    def _set_span(self, val: float) -> None:
        self.write(f"SENS{self._ch}:FREQ:SPAN {val:.7f}")
        self.update_lin_traces()

    def _set_center(self, val: float) -> None:
        self.write(f"SENS{self._ch}:FREQ:CENT {val:.7f}")
        self.update_lin_traces()

    def _set_sweep_type(self, val: str) -> None:
        self.write(f"SENS{self._ch}:SWE:TYPE {val}")

    def _set_cw_frequency(self, val: float) -> None:
        self.write(f"SENS{self._ch}:FREQ:CW {val:.7f}")

    def _enable_averaging(self, val: str) -> None:
        self.write(f"SENS{self._ch}:AVER:STAT {val}")

    def _enable_auto_sweep_time(self, val: str) -> None:
        self.write(f"SENS{self._ch}:SWE:TIME:AUTO {val}")
    ...
    def start_sweep(self):
        ...
        self.write(f"SENS{self._ch}:AVER:CLE")
    
    def init_single(self):
        self.write(f"INIT{self._ch}:IMM")
        

    def _get_sweep_data(self, force_polar: bool = False, update: bool = True) -> np.ndarray:
        if update:
            self.start_sweep()
        ...
        with self.root_instrument.timeout.set_to(timeout):
            ...
            if update:
                print(f'taking {self.avg():d} averages')
                for i in range(self.avg()):
                    self.write(f"INIT{self._ch}:IMM; *WAI")
                    if i>0: print(i, end=' ')

            self.write(
                f"CALC{self._ch}:PAR:SEL "
                f"'{self._tracename}'"
            )
            data_str = self.ask(
                f"CALC{self._ch}:DATA?"
    ...
    def _get_cw_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        with self.status.set_to(1):
            ...
            with self.root_instrument.timeout.set_to(timeout):
                self.write(f"INIT{self._ch}:IMM; *WAI")
                data_str = self.ask(f"CALC{self._ch}:DATA? "
                                    f"SDAT")
            data = np.array(data_str.rstrip().split(",")).astype("float64")
            i = data[0::2]
            q = data[1::2]
...
class ZNB(VisaInstrument):
    ...
    TRACE_CLASS = ZNBTrace

    def __init__(
        self,
        name: str,
        address: str,
        use_existing_traces: bool = True,
        reset_channels: bool = False,
        init_s_params: bool = False,
        **kwargs: Any,
    ) -> None:
        ...
        def parse_esr_opc(val):
            val_int = int(val)
            opc_bit_set = bool(val_int & 1)
            return opc_bit_set
            
        self.add_parameter(
            name = 'operation_complete',
            get_cmd = "*OPC;*ESR?",
            get_parser = parse_esr_opc,
        )
        ...
        traces = ChannelList(
            self, "VNATraces", self.TRACE_CLASS, snapshotable=True
        )
        self.add_submodule("traces", traces)
        
        if use_existing_traces:
            channels = self.ask('CONFigure:CHANnel:CATalog?').strip().strip("'").split(",")
            for channel_number in channels[::2]:
                traces = self.ask(f"CONFigure:CHANnel{channel_number}:TRACe:CATalog?").strip().strip("'").split(",")
                for trace_name in traces[1::2]:
                    self.add_trace(trace_name,
                                   int(channel_number),
            ...
            if init_s_params:
                for i in range(1, num_ports + 1):
                    for j in range(1, num_ports + 1):
                        tr_name = "S" + str(i) + str(j)
                        self.add_trace(tr_name)
                self.traces.lock()
    ...
    def add_trace(self, trace_name: str, channel: int, **kwargs: Any) -> None:
        i_channel = len(self.traces) + 1
        trace = self.TRACE_CLASS(self, trace_name, channel, **kwargs)
